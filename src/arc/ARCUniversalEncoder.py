...
from src.arc.ARCNetworks import UniversalTransformerEncoder, FullyConnectedLayer, Decoder, AttributeHead, PreProcessor
...
class MultiTaskEncoder(L.LightningModule):
    def __init__(
        ...
        self.online_encoder = TensorDictModule(
            UniversalTransformerEncoder(**network_dimensions["Encoder"]),
    ...
    def _forward_grid(self, grid_1, grid_2):
        ...
        results_dict['embedding:original'] = self.online_encoder(grid_1)[:,0,:]
        results_dict['embedding:augmentation'] = self.target_encoder(grid_2)[:,0,:]
